...
func NewJWTService(secretKey string, accessTokenTTL, refreshTokenTTL time.Duration) usecases.JWTService {
...
func (j *JWTServiceImpl) GenerateAccessToken(userID uuid.UUID, role entities.UserRole) (string, error) {
	now := time.Now()
...
// ValidateToken validates a token and returns claims (alias for ValidateAccessToken)
func (j *JWTServiceImpl) ValidateToken(tokenString string) (*usecases.JWTClaims, error) {
	return j.ValidateAccessToken(tokenString)
}

// ValidateAccessToken validates an access token and returns claims
func (j *JWTServiceImpl) ValidateAccessToken(tokenString string) (*usecases.JWTClaims, error) {
...
func (j *JWTServiceImpl) ValidateRefreshToken(tokenString string) (*usecases.JWTClaims, error) {
	token, err := jwt.ParseWithClaims(tokenString, &Claims{}, func(token *jwt.Token) (interface{}, error) {
		if _, ok := token.Method.(*jwt.SigningMethodHMAC); !ok {
			return nil, fmt.Errorf("unexpected signing method: %v", token.Header["alg"])
		}
